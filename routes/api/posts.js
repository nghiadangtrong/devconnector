...
router.get('/', (req, res) => {
    Post
        ...
        .catch(error => res.status(404).json({postnotfound: "No posts found"}));
})
...
router.get('/:id', (req, res) => {
    Post
        ...
        .then(posts => res.json(posts))
        .catch(error => res.status(404).json({postnotfound: "No posts found with that ID"}));
})
        ...
        .then(profile => {
            Post.findById(req.params.id)
                .then(post => {
                    ...
                    // add user id to likes arrays
                    const removeIndex = post.likes.indexOf(item => item.user.toString() === req.user.id);
    ...
    Post.findById(req.params.id)
        .then(post => {
            const removeIndex = post
                .comments
                .findIndex(comment => comment._id.toString() === req.params.comment_id);
